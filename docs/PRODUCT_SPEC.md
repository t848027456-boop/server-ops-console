...
自动发现的 Compose/systemd 资产用于回答“服务器实际上运行了什么”；已登记项目用于回答“哪些业务已确认纳管、检查和操作”。发现资产不得自动获得操作权限。
...
- 按服务器自动发现 Docker/Compose 容器和运行中的 systemd 服务，保留采集时间与截断状态。
- 将只读发现资产与已登记项目分开展示，未登记资产不能直接执行操作。
...
- Compose/systemd 运行资产自动发现，并与已登记项目保持权限隔离。
- 项目登记、健康检查、日志和告警。
- 只读权限和审计基础。
