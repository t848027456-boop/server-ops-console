...
type RuntimeFilter = "all" | "healthy" | "issues";

type RuntimeSelection =
  | { kind: "docker"; server: Server; container: DockerInventoryContainer; registeredProject: Project | null }
  | { kind: "systemd"; server: Server; service: SystemdInventoryService; registeredProject: Project | null };
...
function SummaryStrip({ overview }: { overview: Overview }) {
  const runtime = overview.runtimeInventory;
  return <section className="summary-strip" aria-label="运行概况">
    ...
    {runtime ? <div className="summary-item summary-item--runtime"><div className="summary-icon summary-icon--teal"><Box size={18} /></div><div><strong>{runtime.compose.running} / {runtime.compose.containers}</strong><span>容器运行中</span></div><span className={`delta${runtime.servers.stale ? " delta--warn" : ""}`}>{runtime.compose.groups} 组 · {runtime.systemd.active} 个 systemd 活跃 · {runtime.servers.fresh} / {runtime.servers.total} 台清单新鲜</span></div> : null}
  </section>;
}
...
function runtimeSearchHit(values: Array<string | null | undefined>, query: string) {
  const normalized = query.trim().toLowerCase();
  return !normalized || values.some((value) => value?.toLowerCase().includes(normalized));
}

function runtimeMatchesFilter(value: string, filter: RuntimeFilter) {
  if (filter === "all") return true;
  return filter === "healthy" ? runtimeTone(value) === "healthy" : runtimeTone(value) !== "healthy";
}

function registeredProjectForContainer(container: DockerInventoryContainer, projects: Project[]) {
  return projects.find((project) => {
    ...
    const process = config.process && typeof config.process === "object" && !Array.isArray(config.process) ? config.process as Record<string, unknown> : {};
    return project.type.toLowerCase().includes("compose") && (
      (container.composeProject && process.composeProject === container.composeProject)
      || (project.workingDirectory && container.workingDirectory && project.workingDirectory === container.workingDirectory)
    );
  }) || null;
}

function registeredProjectForSystemd(service: SystemdInventoryService, projects: Project[]) {
  return projects.find((project) => {
    const config = project.config && typeof project.config === "object" && !Array.isArray(project.config) ? project.config as Record<string, unknown> : {};
    const process = config.process && typeof config.process === "object" && !Array.isArray(config.process) ? config.process as Record<string, unknown> : {};
    return project.type.toLowerCase() === "systemd" && (process.unit === service.unit || project.name === service.unit);
  }) || null;
}

function ComposeInventoryView({ servers, projects, query, filter, onSelect }: { servers: Server[]; projects: Project[]; query: string; filter: RuntimeFilter; onSelect: (selection: RuntimeSelection) => void }) {
  const scoped = servers.map((server) => {
    const stale = server.health === "offline" || !server.runtimeInventoryFresh;
    return { server, containers: (server.runtimeInventory?.docker.containers || []).filter((container) => runtimeMatchesFilter(stale ? "stale" : (container.health && container.health !== container.state ? container.health : container.state), filter) && runtimeSearchHit([server.name, server.address, container.name, container.composeProject, container.composeService, container.image, container.ports, container.workingDirectory], query)) };
  }).filter(({ server, containers }) => containers.length > 0 || (!query.trim() && (filter === "all" || filter === "issues") && !server.runtimeInventory));
  if (!servers.length) return <EmptyState title="没有可盘点的服务器" detail="接入 Agent 后，Docker 和 Compose 运行清单会随心跳自动上报。" />;
  if (!scoped.length) return <EmptyState title="没有符合条件的 Compose 服务" detail="调整搜索词或状态筛选，查看其他自动发现资产。" />;
  return <div className="runtime-groups">{scoped.map(({ server, containers }) => {
    const inventory = server.runtimeInventory;
...
function SystemdInventoryView({ servers, projects, query, filter, onSelect }: { servers: Server[]; projects: Project[]; query: string; filter: RuntimeFilter; onSelect: (selection: RuntimeSelection) => void }) {
  const scoped = servers.map((server) => {
    const stale = server.health === "offline" || !server.runtimeInventoryFresh;
    return { server, services: (server.runtimeInventory?.systemd.services || []).filter((service) => runtimeMatchesFilter(stale ? "stale" : service.activeState, filter) && runtimeSearchHit([server.name, server.address, service.unit, service.description, service.subState], query)) };
  }).filter(({ server, services }) => services.length > 0 || (!query.trim() && (filter === "all" || filter === "issues") && !server.runtimeInventory));
  if (!servers.length) return <EmptyState title="没有可盘点的服务器" detail="接入 Agent 后，systemd 运行服务会随心跳自动上报。" />;
  if (!scoped.length) return <EmptyState title="没有符合条件的 systemd 服务" detail="调整搜索词或状态筛选，查看其他自动发现资产。" />;
  return <div className="runtime-groups">{scoped.map(({ server, services }) => {
    const inventory = server.runtimeInventory;
    const sortedServices = [...services].sort((left, right) => left.unit.localeCompare(right.unit));
    const stale = server.health === "offline" || !server.runtimeInventoryFresh;
    return <section className="runtime-server-group" key={server.id} aria-label={`${server.name} 的 systemd 服务`}><RuntimeServerHeading server={server} count={services.length} kind="systemd" />
      {!inventory ? <RuntimeInlineEmpty icon={<RefreshCw size={18} />} title="尚未收到运行清单" detail={server.agentConnected ? "Agent 将在下一次心跳自动上报，无需手工登记。" : "Agent 当前未连接，恢复后会自动采集。"} /> : !inventory.systemd.available ? <RuntimeInlineEmpty icon={<Terminal size={18} />} title="systemd 清单不可用" detail="Agent 未能读取 systemd 服务清单，请检查 systemd 权限或运行状态。" /> : !sortedServices.length ? <RuntimeInlineEmpty icon={inventory.systemd.truncated ? <AlertTriangle size={18} /> : <CheckCircle2 size={18} />} title={inventory.systemd.truncated ? "systemd 清单已截断" : "没有发现 systemd 服务"} detail={inventory.systemd.truncated ? "本次采集超出安全上限，无法确认完整服务数量。" : "本次只读盘点已完成，当前清单为空。"} /> : <><div className="runtime-table-wrap"><table className="runtime-table systemd-runtime-table"><caption className="sr-only">{server.name} 自动发现的 systemd 服务</caption><thead><tr><th>Unit</th><th>说明</th><th>Active</th><th>Sub</th></tr></thead><tbody>{sortedServices.map((service: SystemdInventoryService) => <tr key={service.unit} className="runtime-row--interactive" onClick={() => onSelect({ kind: "systemd", server, service, registeredProject: registeredProjectForSystemd(service, projects) })} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelect({ kind: "systemd", server, service, registeredProject: registeredProjectForSystemd(service, projects) }); }}><td data-label="Unit"><div className="runtime-primary"><strong>{service.unit}</strong></div></td><td data-label="说明"><span className="runtime-description" title={service.description}>{service.description || "未提供说明"}</span></td><td data-label="Active"><RuntimeStatus value={service.activeState} tone={stale ? "unknown" : undefined} /></td><td data-label="Sub"><RuntimeStatus value={service.subState} tone={stale ? "unknown" : runtimeTone(service.activeState) === "healthy" && service.subState.trim().toLowerCase() === "exited" ? "healthy" : undefined} /></td></tr>)}</tbody></table></div>{inventory.systemd.truncated ? <div className="runtime-truncated"><AlertTriangle size={15} /><span>systemd 清单达到采集上限，当前显示的是安全截断后的结果。</span></div> : null}</>}
    </section>;
  })}</div>;
}

function RuntimeDetailDrawer({ selection, onClose, onRegister, onProject }: { selection: RuntimeSelection; onClose: () => void; onRegister: (selection: RuntimeSelection) => void; onProject: (project: Project) => void }) {
  const isDocker = selection.kind === "docker";
  const runtimeName = isDocker ? (selection.container.composeService || selection.container.name) : selection.service.unit;
  const state = isDocker ? selection.container.state : selection.service.activeState;
  const registered = selection.registeredProject;
  return <div className="drawer-shell" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="drawer runtime-detail-drawer" role="dialog" aria-modal="true" aria-label={`${runtimeName}运行详情`}><div className="drawer-head"><div className="drawer-title"><span className="project-glyph project-glyph--large">{isDocker ? <Box size={20} /> : <Terminal size={20} />}</span><div><h2>{runtimeName}</h2><span>{selection.server.name} · {isDocker ? "Compose / Docker" : "systemd"}</span></div></div><IconButton label="关闭" onClick={onClose}><X size={19} /></IconButton></div><div className="drawer-body"><div className="identity-row"><RuntimeStatus value={state} tone={selection.server.health === "offline" || !selection.server.runtimeInventoryFresh ? "unknown" : undefined} /><span className="muted">{selection.server.runtimeInventory ? `采集于 ${relativeTime(selection.server.runtimeInventory.collectedAt)}` : "尚未采集"}</span></div><section className="drawer-section"><div className="section-title"><h3>运行详情</h3><span>Agent 自动发现 · 只读</span></div><dl className="details-list runtime-details-list">{isDocker ? <><div><dt>容器名称</dt><dd>{selection.container.name || "未上报"}</dd></div><div><dt>镜像</dt><dd><code>{selection.container.image || "未上报"}</code></dd></div><div><dt>端口映射</dt><dd><code>{selection.container.ports || "无发布端口"}</code></dd></div><div><dt>重启次数</dt><dd>{selection.container.restartCount}</dd></div><div><dt>工作目录</dt><dd><code>{selection.container.workingDirectory || "未上报"}</code></dd></div><div><dt>Compose 文件</dt><dd>{selection.container.configFiles.length ? selection.container.configFiles.join("、") : "未上报"}</dd></div></> : <><div><dt>Unit</dt><dd><code>{selection.service.unit}</code></dd></div><div><dt>说明</dt><dd>{selection.service.description || "未提供说明"}</dd></div><div><dt>Active</dt><dd>{selection.service.activeState || "未上报"}</dd></div><div><dt>Sub</dt><dd>{selection.service.subState || "未上报"}</dd></div></>}</dl></section><section className="drawer-section"><div className="section-title"><h3>纳管关系</h3><span>{registered ? "已登记项目" : "自动发现资产"}</span></div>{registered ? <button className="runtime-linked-project" type="button" onClick={() => onProject(registered)}><span className="project-glyph"><Code2 size={16} /></span><span><strong>{registered.name}</strong><small>{registered.type} · 允许动作 {registered.allowedActions.length}</small></span><ChevronRight size={16} /></button> : <div className="runtime-unregistered-note"><Eye size={15} /><span>该服务尚未登记为项目，当前不会获得重启、发布或 Shell 权限。</span></div>}</section></div><div className="drawer-actions">{registered ? <button className="button button--secondary" type="button" onClick={() => onProject(registered)}><Eye size={16} />查看登记项目</button> : <button className="button button--primary" type="button" onClick={() => onRegister(selection)}><Plus size={16} />登记为项目</button>}</div></aside></div>;
}

function ProjectsView({ projects, servers, onSelect, onPreflight, onRegister }: { projects: Project[]; servers: Server[]; onSelect: (project: Project) => void; onPreflight: (project: Project) => void; onRegister: (selection?: RuntimeSelection) => void }) {
  ...
  const composeRuntimeIssues = scopedServers.reduce((total, server) => total + (server.runtimeInventory?.docker.containers.filter((container) => server.health === "offline" || !server.runtimeInventoryFresh || runtimeTone(container.health && container.health !== container.state ? container.health : container.state) !== "healthy").length || 0), 0);
  const systemdRuntimeIssues = scopedServers.reduce((total, server) => total + (server.runtimeInventory?.systemd.services.filter((service) => server.health === "offline" || !server.runtimeInventoryFresh || runtimeTone(service.activeState) !== "healthy").length || 0), 0);
  const runtimeTotal = view === "compose" ? composeCount : systemdCount;
  const runtimeHealthy = view === "compose" ? composeHealthy : systemdHealthy;
  const runtimeIssues = view === "compose" ? composeRuntimeIssues : systemdRuntimeIssues;
...
function ProjectModal({ servers, initialSelection, onClose, onCreated }: { servers: Server[]; initialSelection?: RuntimeSelection | null; onClose: () => void; onCreated: () => Promise<void> }) {
  const initialForm = (() => {
    if (!initialSelection) return { id: "", name: "", serverId: servers[0]?.id || "", type: "Compose", branch: "", domain: "", workingDirectory: "", composeProject: "", composeFile: "compose.yaml", unit: "", healthUrl: "", rollbackRef: "" };
    if (initialSelection.kind === "docker") {
      const base = initialSelection.container.composeService || initialSelection.container.name || "service";
      const id = base.toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64) || "service";
      return { id, name: base, serverId: initialSelection.server.id, type: "Compose", branch: "", domain: "", workingDirectory: initialSelection.container.workingDirectory || "", composeProject: initialSelection.container.composeProject || id, composeFile: initialSelection.container.configFiles[0] || "compose.yaml", unit: "", healthUrl: "", rollbackRef: "" };
    }
    const base = initialSelection.service.unit.replace(/\.service$/i, "") || "service";
    const id = base.toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64) || "service";
    return { id, name: base, serverId: initialSelection.server.id, type: "systemd", branch: "", domain: "", workingDirectory: "", composeProject: "", composeFile: "compose.yaml", unit: initialSelection.service.unit, healthUrl: "", rollbackRef: "" };
  })();
...
function AuthenticatedConsole({ onLogout }: { onLogout: () => void }) {
  ...
  const [projectFormSelection, setProjectFormSelection] = useState<RuntimeSelection | null>(null);
  ...
  useEffect(() => {
    ...
    return () => { if (timer !== null) window.clearTimeout(timer); document.removeEventListener("visibilitychange", onVisibilityChange); loadSequence.current += 1; if (toastTimer.current) window.clearTimeout(toastTimer.current); };
  }, [loadAll]);
